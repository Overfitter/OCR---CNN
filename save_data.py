...
img = np.asmatrix(pixel_vals[50]).reshape(16,8).astype(float)
plt.imshow(img)
plt.colorbar()
plt.show()

img = np.asmatrix(pixel_vals[1000]).reshape(16,8).astype(float)
plt.imshow(img)
plt.colorbar()
plt.show()

###################################
#Make Labels categorical -02
###################################
...
#Saving thelabelencoder for future use
